...
export function useWhaleWebSocket() {
    ...
    const connect = () => {
      ...
      try {
        ...
        ws.onmessage = (event) => {
          ...
          try {
            ...
            if (message.type === "initial_data") {
              console.log("📦 Received initial data:", message.data?.length || 0, "alerts");
              setInitialAlerts(message.data || []);
            } else if (message.type === "whale_alert" && message.data) {
              console.log("🐋 New whale:", message.data.symbol, "$" + Math.round(message.data.usd_value).toLocaleString());
        ...
        ws.onclose = (event) => {
          ...
          // Only reconnect if still mounted and wasn't intentional close
  ...
  return { alerts, isConnected, lastUpdate };
}
